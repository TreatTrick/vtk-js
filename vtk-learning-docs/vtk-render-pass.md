...
![渲染介绍](./images/rendering.png)

思考一下，如果camera到(0,1,0)位置，最后颜色缓冲是什么样子？
...
![渲染三角形](./images/triangle.png)

对于这种情况，openGL会自动帮我们插值三角形的其他部分的颜色，比如红和蓝会插值出紫色。对于正中间的像素，应该是白色，但是因为这像素只有一半在三角形里面所以为了去除锯齿感，会变为灰色（纯白色的一半亮度）。其实边缘的红绿也应该是一半亮度的，这里图方便，没有画那么细致。
...
vtkjs 就是基于上述思想创建多个不同的类，每个类负责管理渲染过程中的某一部分职责。保证类的职责单一，易于维护和扩展。当然，vtkjs不是完全和上面的概念一一对应的。
