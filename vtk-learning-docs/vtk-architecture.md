...
场景图是一个层次化的数据结构，用于组织和管理3D场景中的对象。节点树的结构大致如下，接下来会详细解释。
...
```
有了拍摄的机位，我们还需要添加要渲染的实体，也就是两个立方体，这两个立方体就是`Actor`，vtk将其形象的称之为演员。
...
#### 抽象和具体
在 vtk.js 中，场景图不仅负责空间层次的管理，更重要的是实现了**抽象渲染对象**与**具体实现对象**之间的映射关系。
...
`vtkProp`是所有场景图中抽象节点的基类，上述提到的所有`Renderer`,`Actor`,`Mapper`都是继承子`vtkProp`。
